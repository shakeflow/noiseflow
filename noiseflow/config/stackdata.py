...
from noiseflow.utils.timestamp import time_linspace
...
class StackData_Class(object):
    ...
    def save(
        ...
        format="npz",
        compression=False,
        h5_compression_format="gzip",
        h5_compression_opts=3,
    ):
            ...
            with h5py.File(save_path, "w") as f:
                ...
                if compression:
                    group.create_dataset(
                        ...
                        compression=h5_compression_format,
    ...
    def plot(
        ...
        if pair_indx >= self.stack_data.shape[0]:
            raise ValueError(
                "pair_indx must be smaller than stack_data.shape[0]=%d"
                % self.stack_data.shape[0]
            )
        ...
        win_vector = np.array(
            [t_min + i * stack_interval for i in range(0, stack_win_num)]
        )
        ...
        tp_end = tp_start + len(
            tt
        )  # make sure same length, so do not use round((lag_end+self.maxlag+self.dt)/self.dt)
        ...
        if win_interval < (stack_len - stack_step) * (cc_len - cc_step):
            raise ValueError(
                "win_interval must be larger than (stack_len-stack_step)*(cc_len-cc_step)=%f"
                % (stack_len - stack_step)
                * (cc_len - cc_step)
            )
        if win_interval > t_max - stack_len * (cc_len - cc_step) - t_min:
            raise ValueError(
                "win_interval must be smaller than t_max-stack_len*(cc_len-cc_step)-t_min=%f"
                % (t_max - stack_len * (cc_len - cc_step) - t_min)
            )
        ...
        if filter:
            ...
            for i in range(0, len(win_indx)):
                data[i, :] = bandpass(
                    self.stack_data[pair_indx, win_indx[i], tp_start:tp_end],
                    f1,
                    f2,
                    int(1 / self.dt),
                    corners=corners,
        ...
        if mode == "waveform":
            ...
            for i in range(0, len(ydist_n)):
                ax[0].plot(
                    tt,
                    data_plot[int(ydist_n[i])] + ydist_n[i],
                    "k",
                    linewidth=linewidth,
                )
            ...
            # set x axis
            ax[0].set_title(
                "StackData: pair_indx=%d, filter=[%.2f, %.2f] hz" % (pair_indx, f1, f2)
            )
            ...
            # plot ngood
            (markers, stemlines, baseline) = ax[1].stem(
                ydist_n, ngood_n, orientation="horizontal"
            )
            ...
            if save:
                ...
                fig.savefig(
                    os.path.join(save_path), dpi=dpi, format="pdf", bbox_inches="tight"
                )
                plt.close(fig)
            else:
                plt.show()
            ...
            for i in range(0, ntrace):
                tindx = np.where(
                    ((win_vector - win_start) >= i * win_interval)
                    & ((win_vector - win_start) < (i + 1) * win_interval)
                )[0]
            ...
            ax[0].matshow(
                ndata_plot,
                extent=[lag_start, lag_end, ntrace, 0],
                aspect="auto",
                cmap=cmap,
            )
            ...
            (markers, stemlines, baseline) = ax[1].stem(
                yy, ngood, orientation="horizontal"
            )
            plt.setp(markers, marker="D", markeredgecolor="orange", markeredgewidth=1.5)
            plt.setp(stemlines, linestyle="-", color="olive", linewidth=linewidth)
            if ngood_label:
    ...
    def plot_moveout(
        ...
        if source_indx is not None and receiver_indx is not None:
            raise ValueError(
                "source_indx and receiver_indx cannot be specified at the same time"
            )
        elif source_indx is not None:
            if source_indx not in corr_pair[:, 0]:
                raise ValueError("source_indx must be in corr_pair[:,0]")
        elif receiver_indx is not None:
        ...
        if dist_start < np.min(pair_dist):
            raise ValueError(
                "dist_start must be >= (np.min(pair_dist)=%f)" % np.min(pair_dist)
            )
        ...
        if dist_end > np.max(pair_dist):
            raise ValueError(
                "dist_end must be <= (np.max(pair_dist)=%f)" % np.max(pair_dist)
            )
        ...
        if dist_interval < np.diff(np.sort(dist)).min():
            raise ValueError(
                "win_interval must be >= (np.diff(np.sort(dist)).min()=%f)"
                % np.diff(np.sort(dist)).min()
            )
        ...
        if filter:
            ...
            for i in range(0, len(id)):
                data[i, :] = bandpass(
                    self.stack_data[id[i], win_num, tp_start:tp_end],
                    f1,
                    f2,
                    int(1 / self.dt),
                    corners=corners,
        ...
        if mode == "waveform":
            if data.size != 0:
                ...
                for i in range(0, data_plot.shape[0]):
                    ax.plot(
                        tt, amp_scale * data_plot[i] + dist[i], "k", linewidth=linewidth
                    )
                ...
                if source_indx is not None:
                    ax.set_title(
                        "StackData: source_indx=%d, win_num=%d, filter=[%.2f, %.2f] hz"
                        % (source_indx, win_num, f1, f2)
                    )
                elif receiver_indx is not None:
                    ax.set_title(
                        "StackData: receiver_indx=%d, win_num=%d, filter=[%.2f, %.2f] hz"
                        % (source_indx, win_num, f1, f2)
                    )
                else:
                    raise ValueError("source_indx or receiver_indx must be specified")
            ...
            if data.size != 0:
                ...
                yy_tick = np.linspace(0, ntrace, num=yticklabel_num)
                yy_label = np.around(
                    np.linspace(dist_start, dist_end, num=yticklabel_num), decimals=2
                )
    ...
    def plot_moveout_all(
        ...
        if dist_start < np.min(pair_dist):
            raise ValueError(
                "dist_start must be >= (min(pair_dist)=%f)" % np.min(pair_dist)
            )
        ...
        if dist_end > np.max(pair_dist):
            raise ValueError(
                "dist_end must be <= (max(pair_dist)=%f)" % np.max(pair_dist)
            )
        ...
        if mode == "waveform":
            if data.size != 0:
                ...
                ax.legend(loc="upper right", fontsize=8, shadow=False)
                ax.set_title(
                    "StackData: win_num=%d, filter=[%.2f, %.2f] hz" % (win_num, f1, f2)
                )
