...
from noiseflow.utils.timestamp import get_timestamp, time_linspace
...
class CorrData_Class(object):
    ...
    def plot(
        ...
        if pair_indx >= self.corr_data.shape[0]:
            raise ValueError(
                "pair_indx must be <= (corr_data.shape[0]=%d)" % self.corr_data.shape[0]
            )
        ...
        # init timestamp
        t_max, win_interval_raw, win_vector = get_timestamp(
            win_num=self.corr_data.shape[1], cc_len=cc_len, cc_step=cc_step, t_min=t_min
        )
        ...
        if filter:
            ...
            for i in range(0, len(win_indx)):
                data[i, :] = bandpass(
                    self.corr_data[pair_indx, win_indx[i], tp_start:tp_end],
                    f1,
                    f2,
                    int(1 / self.dt),
                    corners=corners,
        ...
        if mode == "waveform":
            ...
            for i in range(0, len(ydist_n)):
                ax.plot(
                    tt, data_plot[ydist_n[i]] + ydist_n[i], "k", linewidth=linewidth
                )

            # set x axis
            ax.set_title(
                "CorrData: pair_indx=%d, filter=[%.2f, %.2f] hz" % (pair_indx, f1, f2)
            )
