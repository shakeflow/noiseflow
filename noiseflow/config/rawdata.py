...
class RawData_Class(object):
    ...
    def decimate(
        self, sampling_rate_new, flag=False, flag_gap=None, threads=1, py=False
    ):
        results = decimate(
            self.data,
            self.sampling_rate,
    ...
    def save(
        ...
        if format == "npz":
            if compression:
                np.savez_compressed(
                    save_path, data=self.data, sampling_rate=self.sampling_rate
                )
            else:
                np.savez(save_path, data=self.data, sampling_rate=self.sampling_rate)
