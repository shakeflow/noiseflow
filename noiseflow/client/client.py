...
class downloader_https:
    ...
    def check_url(self):
        ...
        res = requests.head(
            ...
            verify=False,
        )  # verify=False 关闭ssl双向验证，解决访问https报错问题
    ...
    def download(self):
        ...
        if self.accept_ranges != "bytes":
            if self.show_info:
                print(
                    f"--- Mission ---: {self.url} download from scratch || with single thread, do not support breakpoint resuming"
                )
            ...
            if self.resume:
                ...
                if self.show_info:
                    print(
                        f"--- Mission ---: {self.url} download from scratch || with {self.num_threads} threads, support breakpoint resuming"
                    )
                ...
                if file_path.exists():
                    ...
                    if self.show_info:
                        print(
                            f"--- Mission ---: {self.url} download from breakpoint || with {self.num_threads} threads, support breakpoint resuming"
                        )
                    ...
                    if start == self.content_length:
                        if self.show_info:
                            print(
                                f"--- File ---: {self.filename} has already been downloaded completely"
                            )
                        return
                else:
                    open(file_path, "w+").close()
            ...
            with ThreadPoolExecutor(max_workers=self.num_threads) as pool:
                res = [
                    pool.submit(self.download_by_piece, r)
