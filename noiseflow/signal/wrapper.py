...
from noiseflow.signal.python.filter import (
    bandpass_py,
    bandstop_py,
    highpass_py,
    lowpass_py,
...
def detrend(data, type="linear", flag=False, flag_gap=None, threads=1, py=False):
    ...
    if py:
        ...
        if data.dtype == np.dtype(np.float32) and NOISEFLOW_USE_CPP:
            ...
            raise TypeError(
                "data.dtype must be np.float32, np.float64 or NOISEFLOW_USE_CPP=False"
            )
...
def taper(
    ...
    if type not in [
        ...
        raise ValueError(
            "type must be 'hann', 'hamming', 'blackman', 'blackman_harris', 'gaussian', 'triangular', 'bartlett', 'cosine', 'cosine_np', 'bartlett_hann', 'bohman', 'lanczos', 'flattop', 'kaiser'"
        )
    ...
    if py:
        ...
        if data.dtype == np.dtype(np.float32) and NOISEFLOW_USE_CPP:
            signal_share.taper_float(
                data, max_percentage, type, side, flag, flag_gap, threads
            )
        elif data.dtype == np.dtype(np.float64) and NOISEFLOW_USE_CPP:
            signal_share.taper_double(
                data, max_percentage, type, side, flag, flag_gap, threads
            )
        else:
            raise TypeError(
...
def bandpass(
    ...
    if py:
        ...
        if data.dtype == np.dtype(np.float32) and NOISEFLOW_USE_CPP:
            signal_share.bandpass_float(
                data, freqmin, freqmax, df, corners, zerophase, flag, flag_gap, threads
            )
        elif data.dtype == np.dtype(np.float64) and NOISEFLOW_USE_CPP:
            signal_share.bandpass_double(
                data, freqmin, freqmax, df, corners, zerophase, flag, flag_gap, threads
            )
        else:
            raise TypeError(
...
def bandstop(
    ...
    if py:
        ...
        if data.dtype == np.dtype(np.float32) and NOISEFLOW_USE_CPP:
            signal_share.bandstop_float(
                data, freqmin, freqmax, df, corners, zerophase, flag, flag_gap, threads
            )
        elif data.dtype == np.dtype(np.float64) and NOISEFLOW_USE_CPP:
            signal_share.bandstop_double(
                data, freqmin, freqmax, df, corners, zerophase, flag, flag_gap, threads
            )
        else:
            raise TypeError(
...
def lowpass(
    ...
    if py:
        ...
        if data.dtype == np.dtype(np.float32) and NOISEFLOW_USE_CPP:
            signal_share.lowpass_float(
                data, freq, df, corners, zerophase, flag, flag_gap, threads
            )
        elif data.dtype == np.dtype(np.float64) and NOISEFLOW_USE_CPP:
            signal_share.lowpass_double(
                data, freq, df, corners, zerophase, flag, flag_gap, threads
            )
        else:
            raise TypeError(
...
def highpass(
    ...
    if py:
        ...
        if data.dtype == np.dtype(np.float32) and NOISEFLOW_USE_CPP:
            signal_share.highpass_float(
                data, freq, df, corners, zerophase, flag, flag_gap, threads
            )
        elif data.dtype == np.dtype(np.float64) and NOISEFLOW_USE_CPP:
            signal_share.highpass_double(
                data, freq, df, corners, zerophase, flag, flag_gap, threads
            )
        else:
            raise TypeError(
...
def decimate(data, df, df_new, flag=False, flag_gap=None, threads=1, py=False):
    ...
    if py:
        ...
        if data.dtype == np.dtype(np.float32) and NOISEFLOW_USE_CPP:
            results = signal_share.decimate_float(
                data, df, df_new, flag, flag_gap, threads
            )
        elif data.dtype == np.dtype(np.float64) and NOISEFLOW_USE_CPP:
            results = signal_share.decimate_double(
                data, df, df_new, flag, flag_gap, threads
            )
        else:
            raise TypeError(
