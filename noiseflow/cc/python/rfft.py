...
from noiseflow.cc.python.utils import moving_ave, slice_window, split, whiten
...
class RFFTClass_python(object):
    def __init__(
        ...
        if self.raw_data.dtype == np.dtype(np.float32):
            self.output_data = np.empty(
                (self.channel_num, self.win_num, self.rfft_npts), dtype=np.complex64
            )
            self.time_dtype = np.float32
            self.freq_dtype = np.complex64
        elif self.raw_data.dtype == np.dtype(np.float64):
            self.output_data = np.empty(
                (self.channel_num, self.win_num, self.rfft_npts), dtype=np.complex128
            )
    ...
    def freq_norm(self, rfft_data):
        ...
        if self.freq_norm_method == "no":
            ...
            freq_norm_data = whiten(
                rfft_data,
                self.dt,
                self.freq_norm_method,
                self.freqmin,
                self.freqmax,
                self.smoothspect_N,
                self.whiten_npad,
