...
def rfft(
    ...
    if freqmax > (1 / (2 * dt)):
        raise ValueError(
            "freqmax must be <= (%fhz) according to fmax=1/(2*dt) formula"
            % (1 / (2 * dt))
        )
    ...
    if cc_len > raw_data.shape[1] * dt:
        raise ValueError(
            "cc_len must be <= (raw_data.shape[1]*dt=%f)" % (raw_data.shape[1] * dt)
        )
    ...
    if threads > raw_data.shape[0]:
        raise ValueError(
            "threads must be <= (raw_data.shape[0]=%d)" % raw_data.shape[0]
        )
    ...
    # run rfft
    if py:
        if raw_data.dtype == np.dtype(np.float32) or raw_data.dtype == np.dtype(
            np.float64
        ):
            m = RFFTClass_python(
                raw_data,
        ...
        if raw_data.dtype == np.dtype(np.float32) and NOISEFLOW_USE_CPP:
            ...
            rfft_data = cc_share.rfft_double(
                raw_data,
                dt,
                cc_len,
                cc_step,
            ...
            raise TypeError(
                "raw_data.dtype must be np.float32, np.float64 or NOISEFLOW_USE_CPP=False"
            )

    # generate RFFTData object
    RFFTData = RFFTData_Class(
        rfft_data,
        dt,
        cc_len,
        cc_step,
...
def corr(
    ...
    if maxlag > dt * rfft_data.shape[2]:
        raise ValueError(
            "maxlag must be <= (dt*rfft_data.shape[2]=%f)" % (dt * rfft_data.shape[2])
        )

    # check threads
    if threads > rfft_data.shape[0]:
        raise ValueError(
            "threads must be <= (rfft_data.shape[0]=%d)" % rfft_data.shape[0]
        )
    ...
    if threads > corr_pair.shape[0]:
        raise ValueError(
            "threads must be <= (corr_pair.shape[0]=%d)" % corr_pair.shape[0]
        )
    ...
    # run corr
    if py:
        if rfft_data.dtype == np.dtype(np.complex64) or rfft_data.dtype == np.dtype(
            np.complex128
        ):
            m = CorrClass_python(
                rfft_data, dt, corr_method, corr_pair, maxlag, smoothspect_N, flag, jobs
            )
            m.run()
        ...
        if rfft_data.dtype == np.dtype(np.complex64) and NOISEFLOW_USE_CPP:
            ...
            corr_data = cc_share.corr_double(
                rfft_data,
                dt,
                corr_method,
                corr_pair,
            ...
            raise TypeError(
                "rfft_data.dtype must be np.complex64, np.complex128 or NOISEFLOW_USE_CPP=False"
            )

    # generate CorrData object
    CorrData = CorrData_Class(
        corr_data,
        dt,
        corr_method,
        corr_pair,
...
def stack(
    ...
    if stack_method not in [
        ...
        raise ValueError(
            "stack_method must be 'linear', 'pws', 'robust', 'acf', 'nroot', 'selective', 'cluster', or 'tfpws'"
        )

    # check stack_len
    if stack_len > corr_data.shape[1]:
        raise ValueError(
            "stack_len must be <= (corr_data.shape[1]=%d)" % corr_data.shape[1]
        )
    ...
    if threads > corr_data.shape[0]:
        raise ValueError(
            "threads must be <= (corr_data.shape[0]=%d)" % corr_data.shape[0]
        )
    ...
    # run stack
    if py:
        if corr_data.dtype == np.dtype(np.float32) or corr_data.dtype == np.dtype(
            np.float64
        ):
            m = StackClass_python(
                corr_data,
        ...
        if corr_data.dtype == np.dtype(np.float32) and NOISEFLOW_USE_CPP:
            ...
            raise TypeError(
                "corr_data.dtype must be np.float32, np.float64 or NOISEFLOW_USE_CPP=False"
            )

    # generate StackData object
    StackData = StackData_Class(
        stack_data,
        stack_ngood,
        dt,
        stack_method,
